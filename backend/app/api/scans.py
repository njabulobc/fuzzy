...
def _dispatch_scan(scan: models.Scan) -> None:
    """
    Hook for triggering the actual scan execution.

    In a later stage this will call a Celery task that runs the scanner
    (Slither/Echidna/Foundry) inside Docker. For now, it is a no-op so the
    API remains fully functional and the database schema is stable.
    """
    # Intentionally left as a synchronous no-op at this stage.
    return
...
from app import models, schemas
from app.db.session import get_db
from app.services.scanner import execute_scan  # for fallback if Celery is unavailable
...
def _dispatch_scan(scan: models.Scan) -> None:
    ...
    Preferred path:
    - send a Celery task to the "scans" queue.

    Fallback:
    - run the scan synchronously in the current process if Celery/broker
      is not available (useful for local development without a worker).
    """
    try:
        from app.services.tasks import run_scan_task

        # Fire-and-forget Celery task; the worker will run execute_scan(scan.id)
        run_scan_task.delay(scan.id)
    except Exception:
        # Safe fallback for environments without Celery
